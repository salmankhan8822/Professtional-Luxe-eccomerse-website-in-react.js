...
function ProductsList({ season = null, searchTerm = "" }) {
  ...
  const seasonProducts = season
    ? products.filter(
        (product) => product.season?.toLowerCase() === season.toLowerCase(),
      )
    : products;

  // Apply category filter first, then search within the selected season/category
  let filteredProducts = seasonProducts;
        ...
        <div className="container">
          ...
          {searchTerm && (
            <div className="search-results-message mb-4 text-center">
              <h4>
                {sortedProducts.length} results found for "{searchTerm}"
              </h4>
            </div>
          )}
              ...
              <div className="col-6 col-sm-6 col-lg-3" key={product.id}>
                ...
                >
                  ...
                  <div className="summer-product-image">
                    ...
                    {/* Sale Badge */}
                    {/* DISCOUNT */}
                    <span className="summer-sale-badge">
                      -{getDiscountPercentage(product.oldPrice, product.price)}%
                    </span>
                    {/* Sold Quantity */}
                    <span className="summer-sold-badge">
                      {product.sold} sold
                    </span>
                  </div>
                  ...
                  <div className="summer-product-info">
                    <p className="summer-product-category">
                      {product.category}
                    </p>

                    <h3 className="summer-product-name">{product.name}</h3>

                    <p className="summer-product-description">
                      {product.description}
                    </p>
